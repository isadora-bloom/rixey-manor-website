...
const PACKAGES = [
  {
    ...
    subtitle: 'Friday 3pm to Sunday 10am',
    summary: 'Rehearsal dinner Friday, wedding Saturday, brunch and exit Sunday — 10am check-out, or 1pm with the brunch upgrade. Two nights of lodging for 14. The whole estate, the whole weekend.',
...
export default function PricingCalculator() {
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      addons,
      // Itemized price math for the couple-facing email. Keys mirror result{}.
      breakdown: result ? {
        base:             Math.round(result.base),
        satMod:           Math.round(result.satMod),
        friMod:           Math.round(result.friMod),
        upgradeAmt:       Math.round(result.upgradeAmt),
        ...
        subAfterDiscount: Math.round(result.subAfterDisc),
